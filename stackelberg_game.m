function [bs_income,uav_income,user_outcome] = stackelberg_game(bs,uav,user,epoch)
...
[local,offload_bs,offload_uav,offload_relay]=deal(0.25*ones(user_num,1),0.25*ones(user_num,1),0.25*ones(user_num,1),0.25*ones(user_num,1)); % Ð¶ÔØ±ÈÀý³õÊ¼»¯
...
%                 F_i(b,i)=(offload_bs(i)+offload_relay(i)) * D(i);
    ...
    for u = 1:uav_num
        ...
        for i = 1:user_num
            ...
            if ~converge(i)
%                 f_i(u,i) = offload_uav(i) * D(i);
                if u == selected_uav(i) && offload_uav(i) > 0
                    m_i_low(u,i) = m_i(u,i);
...
%         end
        ...
        if episode > 50
            ...
            if rate_cha_1 <= 0.05 && rate_cha_2 <= 0.05
                converge(i) = true;
            end
        end
    end
